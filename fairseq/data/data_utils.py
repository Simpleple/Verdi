...
def load_indexed_dataset(path, dictionary, dataset_impl=None, combine=False, default='cached', path_xml=None):
...
def load_word_qe_tags(file):
    tags = []
    with open(file, 'r', encoding='utf-8') as f:
        for line in f:
            tag = [int(t) for t in line.strip().split()]
            tags.append(tag)
    return tags


def load_bpe_tags(file):
    tags = []
    with open(file, 'r', encoding='utf-8') as f:
        for line in f:
            tag = eval(line)
