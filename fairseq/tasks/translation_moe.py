...
class TranslationMoETask(TranslationTask):
    ...
    def __init__(self, args, src_dict, tgt_dict):
        ...
        if args.bidirection_decoder > 0:
            self.bidirection_decoder = True
        else:
            self.bidirection_decoder = False
    ...
    def _get_loss(self, sample, model, criterion):
        ...
        def get_lprob_y(encoder_out, prev_output_tokens_k, bw_prev_output_tokens_k=None):
            if bw_prev_output_tokens_k is not None:
                fw_feature, extra = model.decoder.extract_features(prev_output_tokens_k, encoder_out, None)
                bw_feature, extra = model.bw_decoder.extract_features(bw_prev_output_tokens_k, encoder_out, None)
                fbw_feature = model.outputlayer(torch.cat([fw_feature, bw_feature], dim=-1))
                fbw_feature = model.decoder.output_layer(fbw_feature)
                net_output = (fbw_feature, extra)
            else:
        ...
        def flipBatch(data, lengths):
            ...
            for i in range(data.shape[0]):
                data[i, 1:lengths[i]] = data[i, 1:lengths[i]].flip(dims=[0])
        ...
        def get_lprob_yz(winners=None):
            ...
            if winners is None:
                ...
                for i in range(k): # k次解码， start不同
                    ...
                    bw_prev_output_tokens_k = None
                    if self.bidirection_decoder:
                        bw_prev_output_tokens_k = flipBatch(prev_output_tokens_k, sample['tgt_lengths'])
                        bw_prev_output_tokens_k[:, 0] = self.expert_index(i)
                    lprob_y.append(get_lprob_y(encoder_out, prev_output_tokens_k, bw_prev_output_tokens_k=bw_prev_output_tokens_k))
                ...
                if self.bidirection_decoder:
                    bw_prev_output_tokens_k = flipBatch(prev_output_tokens_k, sample['tgt_lengths'])
                    bw_prev_output_tokens_k[:, 0] = self.expert_index(winners)
                lprob_y = get_lprob_y(encoder_out, prev_output_tokens_k, bw_prev_output_tokens_k=bw_prev_output_tokens_k)  # -> B
