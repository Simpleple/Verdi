...
@register_task('translation_moe_dual')
class TranslationMoEDualTask(TranslationTask):
    """
    ...
    def _get_loss(self, sample, model, criterion, prim=True):
        ...
        def get_lprob_y(encoder_out, prev_output_tokens_k):
            ...
            net_output = decoder.decode(prev_output_tokens_k, encoder_out)
            loss, _ = criterion.compute_loss(model, net_output, sample, reduce=False, prim=prim)
        ...
        def get_lprob_yz(winners=None):
            if prim:
                ...
                encoder_out = torch.zeros((sample['net_input']['src_tokens'].shape[1], sample['net_input']['src_tokens'].shape[0], model.encoder.output_embed_dim)).to(sample['target'].device)
                encoder_padding_mask = sample['net_input']['src_tokens'].eq(model.encoder.embed_positions.padding_idx)
                ...
                encoder_out = torch.zeros((sample['target'].shape[1], sample['target'].shape[0], model.encoder.output_embed_dim)).to(sample['target'].device)
                encoder_padding_mask = sample['target'].eq(model.encoder.embed_positions.padding_idx)
                if not encoder_padding_mask.any():
                    encoder_padding_mask = None
                encoder_out = encoder.encode(sample['target'],
                                             encoder_out={'encoder_out': encoder_out, 'encoder_padding_mask': encoder_padding_mask})
            ...
            if winners is None:
                ...
                for i in range(k):
                    if prim:
                        ...
                        prev_output_tokens_k = torch.zeros_like(sample['net_input']['src_tokens']).fill_(model.encoder.embed_positions.padding_idx)
                        target_lens = torch.sum(
                            sample['net_input']['src_tokens'] != model.encoder.embed_positions.padding_idx, dim=1)
                ...
                if prim:
                    ...
                    prev_output_tokens_k = torch.zeros_like(sample['net_input']['src_tokens']).fill_(
                        model.encoder.embed_positions.padding_idx)
                    target_lens = torch.sum(
                        sample['net_input']['src_tokens'] != model.encoder.embed_positions.padding_idx, dim=1)
                    for j in range(prev_output_tokens_k.shape[0]):
                        prev_output_tokens_k[j, 0] = 2
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False, prim=True):
        model.train()
        loss, sample_size, logging_output = self._get_loss(sample, model, criterion, prim=prim)
        if ignore_grad:
            loss *= 0
