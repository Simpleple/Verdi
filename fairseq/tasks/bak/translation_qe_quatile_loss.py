...
class TranslationQETask(TranslationMTask):
    ...
    def get_experts_decoder_output(self, predictor, estimator, sample, xml_model=None, xml_estimator=None):
        ...
        if self.xml_tgt_only:
            ...
            pre_qefv = torch.mul(xml_tensor, target_embeding)
            post_qefv = xml_tensor
            paded_tensor = torch.FloatTensor(xml_tensor.shape[0], xml_tensor.shape[1], 20).fill_(0).cuda()
            # xml_qefv = torch.cat([pre_qefv, post_qefv, pre_qefv-post_qefv, pre_qefv+post_qefv, pre_qefv*post_qefv, paded_tensor], dim=-1)
            xml_qefv = torch.cat([pre_qefv, post_qefv], dim=-1)  # .transpose(0, 1)
            ...
            if self.estimator_xml_only:
                ...
                if xml_estimator is not None:
                    if self.share_estimator:
                        ...
                        ter_prediction = ter_prediction * 0.7 + estimator(xml_qefv, share_estimator=True) * 0.3
                    else:
    ...
    def _get_qe_loss(self, sample, predictor, estimator, criterion, xml_model=None, valid=False, xml_estimator=None, step=None):
        ...
        if step < 400:
            ...
            loss = self.loss_fn(ter_prediction, ter_gt) + self.quantile_loss(ter_prediction, ter_gt, 0.6)
        loss = loss.sum()
        # else:
    ...
    def train_step(self, sample, predictor, estimator, criterion, optimizer, ignore_grad=False, xml_model=None, xml_estimator=None, step=None):
        if xml_estimator is not None:
            xml_estimator.train()
        estimator.train()
    ...
    def valid_step(self, sample, predictor, estimator, criterion, xml_model=None, xml_estimator=None, step=None):
        predictor.eval()
        estimator.eval()
        if xml_estimator is not None:
            xml_estimator.eval()
