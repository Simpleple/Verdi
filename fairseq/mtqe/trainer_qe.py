...
class Trainer(object):
    ...
    def __init__(self, args, task, predictor, estimator, criterion, xml_model=None, xml_estmimator=None, dummy_batch=None, oom_batch=None):
        self.args = args
        self.task = task
        ...
        self.xml_estimator = xml_estmimator
        self.xml_model = xml_model
        self.cuda = torch.cuda.is_available() and not args.cpu
        if args.fp16:
            self._predictor = self._predictor.half()
            self._estimator = self._estimator.half()
            if self.xml_estimator is not None:
                self.xml_estimator = self.xml_estimator.half()
            if self.xml_model is not None:
                self.xml_model = self.xml_model.half()
        if self.cuda:
            self.criterion = self.criterion.cuda()
            self._predictor = self._predictor.cuda()
            self._estimator = self._estimator.cuda()
            if xml_estmimator is not None:
                self.xml_estimator = self.xml_estimator.cuda()
            if self.xml_model is not None:
                self.xml_model = self.xml_model.cuda()
    ...
    def model(self):
        if self._wrapped_model is None:
            ...
            self._wrapped_model = self._estimator
    ...
    def _build_optimizer(self):
        ...
        if self.xml_estimator is not None:
            params_xml = list(filter(lambda p: p.requires_grad, self.xml_estimator.parameters()))
            params.extend(params_xml)
        if self.args.fp16:
    ...
    def _build_estimator_optimizer(self):
        params = list(filter(lambda p: p.requires_grad, self._estimator.parameters()))
        if self.args.fp16:
            if self.cuda and torch.cuda.get_device_capability(0)[0] < 7:
                print('| WARNING: your device does NOT support faster training with --fp16, '
                      'please switch to FP32 which is likely to be faster')
    ...
    def load_predictor_checkpoint(
        ...
        if os.path.exists(filename):
            ...
            try:
                # print(state['model'])
                self.get_predictor().load_state_dict(state['model'], strict=False)
            except Exception:
                raise Exception(
    ...
    def get_train_iterator(self, epoch, combine=True, xml_dico=None, xml_params=None, task='translation_qe'):
        """Return an EpochBatchIterator over the training set for a given epoch."""
        print('| loading train data for epoch {}'.format(epoch))
        self.task.load_dataset(self.args.train_subset, epoch=epoch, combine=combine, xml_dico=xml_dico, xml_params=xml_params, task=task)
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self._predictor, self.model, self.criterion, self.optimizer,
                        ignore_grad, xml_model=self.xml_model
                    )
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self._predictor, self.model, self.criterion, xml_model=self.xml_model
                )
            except RuntimeError as e:
                if 'out of memory' in str(e) and not raise_oom:
                    ...
                    logging_output = {
                        ...
                        'posterior': torch.FloatTensor([1] * sample_size).to(sample['id'].device)
                    }
