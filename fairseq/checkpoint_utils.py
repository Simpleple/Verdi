...
def load_estimator_checkpoint(args, trainer,xml_dico=None, xml_params=None):
    """Load a checkpoint and restore the training iterator."""
    # only one worker should attempt to create the required dir
    if args.distributed_rank == 0:
        os.makedirs(args.save_dir, exist_ok=True)

    if os.path.isabs(args.restore_estimator_file):
        checkpoint_path = args.restore_estimator_file
    else:
        checkpoint_path = os.path.join(args.save_dir, args.restore_estimator_file)

    extra_state = trainer.load_estimator_checkpoint(
        checkpoint_path,
    ...
    if extra_state is not None and not args.reset_dataloader:
        ...
        epoch_itr = trainer.get_train_iterator(epoch=itr_state['epoch'], xml_dico=xml_dico, xml_params=xml_params, task=args.task)
        epoch_itr.load_state_dict(itr_state)
    else:
        epoch_itr = trainer.get_train_iterator(epoch=0, xml_dico=xml_dico, xml_params=xml_params, task=args.task)
...
def load_predictor_checkpoint(args, trainer):
    """Load a checkpoint and restore the training iterator."""
    # only one worker should attempt to create the required dir
    if args.distributed_rank == 0:
        os.makedirs(args.save_dir, exist_ok=True)
    ...
    extra_state = trainer.load_predictor_checkpoint(
        checkpoint_path,
        args.reset_optimizer,
        args.reset_lr_scheduler,
        eval(args.optimizer_overrides),
