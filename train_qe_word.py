...
def validate(args, trainer, task, epoch_itr, subsets, xml_estimator=None, evaluate=False):
    ...
    for subset in subsets:
        ...
        for sample in progress:
            if subset == 'train' and not evaluate and len(ter_prediton) > 1000:
                break
            ...
            for k, v in log_output.items():
                if k in ['loss', 'nll_loss', 'ntokens', 'nsentences', 'sample_size']:
                    ...
                    wd_pred, gap_pred = v[0].transpose(0, 1), v[1].transpose(0, 1)
                    wd_pred_int = (wd_pred > 0.5).long()
                    gap_pred_int = (gap_pred > 0.5).long()
                    batch_size = wd_pred.shape[0]
                    for i in range(batch_size):
                        pred, pred_f, gt = [], [], []

                        word_len = torch.sum(sample['word_tags'][i] != -1).item()
                        for j in range(word_len):
                            pred.append(gap_pred_int[i, j].item())
                            pred.append(wd_pred_int[i, j].item())
                            pred_f.append(gap_pred[i, j].item())
                            pred_f.append(wd_pred[i, j].item())
                            gt.append(sample['gap_tags'][i, j].item())
                            gt.append(sample['word_tags'][i, j].item())
                        pred.append(gap_pred_int[i, word_len].item())
                        pred_f.append(gap_pred[i, word_len].item())
                        gt.append(sample['gap_tags'][i, word_len].item())

                        ter_prediton += pred
                        ter_gt += gt
                        pred_sent.append(['OK' if p == 1 else 'BAD' for p in pred])
                        pred_float.append(pred_f)
        ...
        if evaluate:
            ...
            with open(args.save_dir + '/prediction', 'w', encoding='utf8') as fw:
                for id, t in zip(ids, pred_sent):
                    fw.writelines(str(id) + " " + ' '.join(t) + '\n')
            sort_sorce(args.save_dir + '/prediction', args.save_dir + '/' +subset + '_ce-qe-qsm-test.hter.xml')

            with open(args.save_dir + '/prediction', 'w', encoding='utf8') as fw:
                for id, t in zip(ids, pred_float):
                    fw.writelines(str(id) + " " + ' '.join(['%.5f' % f for f in t]) + '\n')
            sort_sorce(args.save_dir + '/prediction', args.save_dir + '/' +subset + '_ce-qe-qsm-test.hter.float.xml')

        metric = compute_scores(ter_gt, ter_prediton)
        print('valid mcc: {}'.format(metric[-1]))
        print('valid f1: {}'.format(metric[0]))
        print('valid f1: {}'.format(metric[1]))
...
def compute_scores(true_tags, test_tags):
    ...
    flat_pred = [1.0 if t > 0.5 else 0.0 for t in test_tags]
    flat_true = true_tags
...
def sort_sorce(filein, fileout):
    ...
    with open(filein, 'r', encoding='utf8') as fr:
         for line in fr.readlines():
             line = line.strip('\n').split(' ')
             # if len(line) == 2:
             score_ids[int(line[0])] = ' '.join(line[1:])
